...
def update_net_sentiment(contents):
    if contents is not None:
        ...
        # transform data to a summary sum_table for scatter plot
        ...
        # create summary table of positive, negative, neutral Sentiment
        sum_table = pd.crosstab([fdata.Month, fdata.Source], fdata.Sentiment,
                               rownames = ['Month', 'Source'],
                               colnames = ['Sentiment'])

        # create summary table contains net Sentiment
        # define empty list to hold values in for loop
        month_list = []
        source_list = []
        net_sent_list = []

        # define sequence for 'for' loop
        ...
        # define the sequence of month for plotting
        ...
        # define the values needed in insights
        source_list = []
        sent_sum = []
...
def update_geographic(contents):
    if contents is not None:
        ...
        ## Create summary table for choropleth graph
        # Use only positive Sentiment
        pos_data = fdata[fdata['Sentiment'] == 'positive']
        # summary of positive sentiment by state for choropleth
        df2 = pos_data.groupby('State').count().reset_index()[['State','Sentiment']]

        # summary top3 count by state by source for insights
